package ADTTree;
//Array Implementation of a minHeap:
public class minHeap {
    private int[] Heap;
    private int size;
    private int maxSize;

    public minHeap(int pMaxSize){
        maxSize = pMaxSize;
        size = 0;
        Heap = new int[pMaxSize+1]; //array of size n+1
        Heap[0] = Integer.MIN_VALUE;
        for(int i=1;i<=pMaxSize;i++) Heap[i] =Integer.MAX_VALUE;
    }
    ...
    public int findMin(){ return Heap[1];}
    ...
    public void removeMin() {
        ...
        size--;
        int n = 1;
        while(!isLeaf(n) && Heap[n]>Math.min(Heap[leftChild(n)],Heap[rightChild(n)])){
